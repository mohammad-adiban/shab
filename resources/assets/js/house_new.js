...
( function () {(function ($) {
    ...
    var ClassConfig = {
        ...
        /**
         * @memberOf
         * @description set title of every day
         * @type {string}
         * @default empty
         */
        dayTitle: 'hassan',
                ...
                renderDays: function (self) {
                    ...
                    var addSpan = function (day, month, year, cssClass) {
                        ...
                        var span = $("<span/>")
                            ...
                            .attr("data-toggle", "tooltip") // alizadeh118
                            .attr("title", '') // alizadeh118
...
$(document).ready(function () {
    ...
    $.ajax({
            ...
            url: "/calendars/show",
            data:{
                house: digitsToLatin($(".house_id").text()),
    ...
    calendar_price_list = [];
    $('.p_min').text(priceToReadable($('.p_min').text()));
    $('.p_med').text(priceToReadable($('.p_med').text()));
    $('.p_max').text(priceToReadable($('.p_max').text()));
    $('.p_extra').text(priceToReadable($('.p_extra').text()));
    $('.min_from').text('از ' + digitsToHindi($('.p_min').text()));

    $('.p_min1').text(priceToReadable($('.p_min1').text()));
    $('.p_med1').text(priceToReadable($('.p_med1').text()));
    $('.p_max1').text(priceToReadable($('.p_max1').text()));
    $('.p_extra1').text(priceToReadable($('.p_extra1').text()));
	traverse($('#price')[0]);
    $(".activeGallery").click(function () {
        $('.gallery_photo').each(function () {
            $(this).replaceWith('<img class="host-imgs" src="'+$(this).val()+'" name="'+$(this).attr('name')+'">');
        });
    ...
    $('#submitBtn, #submitBtn-notLogin').click(function(e) {
        var count;
        var sSelector = '#checkin1';
        var eSelector = "#checkout1";
        ...
        var cinDate = moment.utc(digitsToLatin($(sSelector).val()), 'jYY/jMM/jDD');
        var coutDate = moment.utc(digitsToLatin($(eSelector).val()), 'jYY/jMM/jDD');
        $('input[name="checkin"]').val(cinDate.unix());
        $('input[name="checkout"]').val(coutDate.unix());
        $('input[name="accomodates"]').val($('#guests').val());
    ...
    function make_fixed() {
        ...
        if ($(window).width() > 992) {
            var width = $("#left-column").width();
            var left = $("#left-column").offset().left;
            var top = $("#left-column").offset().top;

            var house_img = $(".house").parent().width();
            $(".house").css({
                //width: house_img + 30
            })
            ...
            $('.house').attr('style', function(i, style)
            {
    ...
    $("#SaveToListButton").change(function () {
        $(".wish-list__heart").toggleClass("selected fa-heart-o fa-heart");
    });

    var checkin, first=true;
    function checkin_date() {
	    $("#checkin1").pDatepicker({
	        format: "YY/MM/DD",
	        persianDigit: true,
	        onShow: function () {
	            console.log('on check show');
	            setTimeout(function () {
	                setDatesPrice();
	            }, 100)
	        },
	        checkDate: function (unix) {
	            ...
	            if (unix < yester) {
	                ...
	                for (var i=0; i<calendar.length; i++) {
	                    if (calendar[i].disabled == 1  || calendar[i].unavailable == 1) {
	                        var date = moment(calendar[i]["date"], 'YYYY/MM/DD').unix() * 1000;
	                        if (unix == date) {
	                            output = false;
	                        }
	                    }
	                    if (calendar[i].disabled == 0 && calendar[i].unavailable == 0) {
	                        var date = moment(calendar[i]["date"], 'YYYY/MM/DD').unix() * 1000;
	                        var d = new persianDate(date);
	                        var obj = {
	                            day: d.date(),
	                            unix: date,
	                        ...
	                        var data = calendar_price_list.find( function(ele) {
	                            return ele["date"] === calendar[i]["date"];
	                        });

	                        if(data) {
	                        } else {
	                            calendar_price_list.push(obj);
	                        }
	                    }
	                }
	            }
	        ...
	        dayPicker: {
	            onSelect: function () {
	                ...
	                $('#checkin1').val(digitsToHindi($('#checkin1').val()));
	                setTimeout(function () {
	                    checkout_date();
	                    setDatesPrice();
	                },100);
    ...
    function checkout_date() {
        $("#checkout1").pDatepicker({
            ...
            dayPicker: {
                onSelect: function () {
                    $('#checkout1').val(digitsToHindi($('#checkout1').val()));
                }
            }
        });
        $("#checkout1").pDatepicker("show");
        first = false;

    }

    // set title for days
    function setDatesPrice() {
        console.log('setDatesPrice called', calendar_price_list);
        $('.datepicker-plot-area td:not(.disabled) span[data-toggle="tooltip"]').each(function () {
            var day = fatoen_digits($(this).text());
            for (var i=0; i<calendar_price_list.length; i++) {
                if (day == calendar_price_list[i].day) { // for even days
                    var price = calendar_price_list[i].price;
                    price = digitsToHindi(commaSeperated(price*1000));
                    price = price + ' ' + 'تومان';
                    $(this).attr("title", price).tooltip();
                }
            }
        });
    }

    // smooth scroll to inner element
    $('.navbar-house__link').click(function () {
        //$('.navbar-house__link').removeClass('active');
    ...
    $('a[href^="#"]:not(a[href="#/"])').click(function () {
        ...
        if ($(dest).offset() != undefined) {
            $('html, body').animate({
                scrollTop: $(dest).offset().top - 39
            }, 500);
        }
    });

    // display booking in mobile
    $(".register-btn-mobile").click(function () {
        $("#left-column").removeClass("hidden-xs").css({
            "position": "fixed",
    ...
    $(".close-btn-mobile").click(function () {
        $("#left-column").animate({
            ...
            $(this).addClass("hidden-xs").removeAttr("style");
        });
        $("body").css("overflow-y", "auto");
    });
    //display share icon in mobile
    $(document).on("click", ".share-btn-mobile .fa-share-alt", function (e) {
        var $this = $(this);
        //e.preventDefault();
        var i = 45;
        $(".share-btn-mobile .social-mobile").each(function () {
            $(this).animate({
                bottom: i,
            })
            i += 35;
        ...
        $this.animate({display: "none"}, 0, function () {
            $this.toggleClass("fa-share-alt fa-close").animate({display: "inline-block"}, "fast");
        });
    });
    $(document).on("click", ".share-btn-mobile .fa-close", function (e) {
        var $this = $(this);
        e.preventDefault();
        $(".share-btn-mobile .social-mobile").each(function () {
            $(this).animate({
    ...
    //add more button to long comments
    $(".comment__text").each(function (i, v) {
        var valid_char_num = 700;
        if ($(this).text().length > valid_char_num) {
            $(this).data('complete', $(this).text());
            $(this).text($(this).text().substr(0, valid_char_num) + "...");
            var more_btn = $("<a>").addClass("comment__more").text("بیشتر");
            var plus_icon = $("<i>").addClass("fa fa-plus");
            more_btn.prepend(plus_icon);
            $(this).append(more_btn);
        }
    })
    $(".comment__more").click(function () {
        var comp = $(this).parent().data("complete");
        $(this).parent().text(comp);
        $(this).parent().removeData("complete");
    });
    ...
    $(".pagination__page__link").click(function () {
        ...
        var current = $(".pagination__page__link.selected").text();
        $(".pagination__page__link").removeClass("selected");
        if ($(this).is(".fa-caret-left")) {
            $(".pagination__page__item:nth-child(" + (Number(current) + 2) + ") > .pagination__page__link").addClass("selected");
        } else if ($(this).is(".fa-caret-right")) {
            $(".pagination__page__item:nth-child(" + (Number(current)) + ") > .pagination__page__link").addClass("selected");
        } else { // click on nums
            $(this).addClass("selected");
        }
        $(".comment").toggleClass("hidden");
        // display caret-right items
        if (Number($(".pagination__page__link.selected").text()) > 1) {
            $(".pagination__page__link.fa-caret-right").removeClass("disabled");
        } else {
            $(".pagination__page__link.fa-caret-right").addClass("disabled");
        }

        // display caret-right items
        if (Number($(".pagination__page__link.selected").parent().is(":nth-last-child(2)"))) {
            $(".pagination__page__link.fa-caret-left").addClass("disabled");
        } else {
            $(".pagination__page__link.fa-caret-left").removeClass("disabled");
        }
    ...
    if($(window).width() >= 992) {
        $(".fixed-left-column").css("margin-right", $(".fixed-left-column").width() / 2 + 5);
    }
	...
	$(window).scroll(function () {
		...
		$('#menu-center a').each(function () {
			...
			if (refElement.offset().top <= scrollPos + 100  && refElement.offset().top + refElement.height() > scrollPos + 100) {
		...
		if (scrollPos >= $(".house-info").offset().top) {
			$(".navbar-house").css({"top": "0px", "box-shadow": "0 -1px 20px rgba(0,0,0,0.8)"});
		} else {
			$(".navbar-house").css({"top": "-100px", "box-shadow": "none"});
		}
		
		if($(window).width() >= 992) {
			if(scrollPos < $("#location").offset().top - $("#location").height() + 200) {
				$(".book__header").removeClass('fixed-left-column-header');
				$(".fixed-left-column").css("position" , "fixed");
				// $(".book_body, .wish-list").css("box-shadow" , "0px 1px 9px #d8d8d8");
				if(scrollPos > 52)
					$(".fixed-left-column").removeClass('p_absolute_52').addClass('p_fixed');
				else if(scrollPos <= 52)
					$(".fixed-left-column").removeClass('p_fixed').addClass('p_absolute_52');
			}
			 else {
				$(".fixed-left-column").removeClass('p_fixed p_absolute_52').css({"top": $("#location").offset().top - $("#location").height() + 240 +"px" , "position":"absolute"});
				$(".book__header").addClass('fixed-left-column-header');
				$(".book__header").css("width" , $(".fixed-left-column").width());
				// $(".book_body, .wish-list").css("box-shadow" , "none");
			 }
...
function fatoen_digits(persian) {
    var persian_digit = persian.split('');
    var english_digit = '';
    for (var i = 0; i < persian_digit.length; i++) {
        var current_dig = persian_digit[i];
        switch (current_dig) {
            case '۰':
                current_dig = 0;
                break;
